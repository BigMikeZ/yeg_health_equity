...
yeg_joined_aggregated |> 
  select(-geography) |> 
  mutate(
    weighted_average_income = weighted_average_income_k * 1000,
    .keep = "unused"
  ) |> 
  tbl_summary(
...
Results of the final regression model using the clustered robust standard error is displayed on the table above. The output shows that household income is significantly related to hypertension prevalence rate in Edmonton (n = 15, p = 0.0056). Specifically, an $1,000 increase in household income is associated with a 0.023 percentage point decrease in hypertension prevalence rate in Edmonton SLGAs.
